...
void codigoWorker(int my_id){
    ...
    MPI_Recv(&(argToThread->my_nroCombinaciones),1,MPI_INT,NODO_MASTER,COMBPORWORKER_TAG,MPI_COMM_WORLD,&status);
    if(argToThread->my_nroCombinaciones > 0){//si es cero se termina inmediatamente la ejecución de este worker
        /*se recibe los parámetros acerca de las matrices a usar*/
        tam_in = 2*sizeof(int) + 2*sizeof(double);
        buf_in = (char*)malloc(tam_in);
        argToThread->infoMaps = (InfoMaps*)malloc(sizeof(InfoMaps));
        MPI_Recv(buf_in,tam_in,MPI_PACKED,NODO_MASTER,MATRIZ_TAG,MPI_COMM_WORLD,&status);
...
void * recibirTrabajos(void * argToThread){
    ...
    i = 0;
    MPI_Recv(&nroDeEnviosARecibir,1,MPI_INT,NODO_MASTER,NRO_ENVIOS_TAG,MPI_COMM_WORLD,&status);
    nroCombinaciones = (int *)malloc(nroDeEnviosARecibir*sizeof(int));
    MPI_Recv(nroCombinaciones,nroDeEnviosARecibir,MPI_INT,NODO_MASTER,COMBINACIONES_TAG,MPI_COMM_WORLD,&status);
    while(nroDeEnviosARecibir > 0){
            tam_buf = nroCombinaciones[i]*NRO_PARAMETROS*sizeof(double);
...
void * enviarResultados(void * argToThread){
    ...
    int cell, col, row, cantCeldas = args->infoMaps->Rows * args->infoMaps->Cols;
    char * out_buff;
    int tam_out, position;
    tam_out = 2*cantCeldas*sizeof(double);//el buffer de salida tiene que tener el espacio suficiente para almacenar dos mapas
    out_buff = (char*)malloc(tam_out);
    ...
    for ( row=0; row < args->infoMaps->Rows; row++ )
    {
        ...
        {
            args->buffer_ignMap[indexBuffer][cell] = ((args->buffer_ignMap[indexBuffer][cell]==INFINITO) ? 0.0 : 1.0);
        }//fin for
    }//fin for
    ...
    for ( row=0; row < args->infoMaps->Rows; row++ )
    {
        for ( cell=row*args->infoMaps->Cols, col=0; col <  args->infoMaps->Cols; col++, cell++ )
        {
            args->buffer_flMap[indexBuffer][cell] = ((args->buffer_flMap[indexBuffer][cell]==INFINITO) ? 0.0 : args->buffer_flMap[indexBuffer][cell]);
        }//fin for
    }//fin for

    /*se empaquetan ambas matrices en un solo paquete*/
    position = 0;
    
    MPI_Pack(args->buffer_ignMap[indexBuffer],cantCeldas,MPI_DOUBLE,out_buff,tam_out,&position,MPI_COMM_WORLD);
    MPI_Pack(args->buffer_flMap[indexBuffer],cantCeldas,MPI_DOUBLE,out_buff,tam_out,&position,MPI_COMM_WORLD);
