...
/**********************************************************************************************************/
void codigoMaster(int my_id, int nproc, char * fileName_maps, char * fileName_param, char * fileName_outIncencio,
 ...
 * Por último tiene que sumar y promediar los resultados retornados por los workers
 Para ello, se usan dos hilos: uno de envio tareas y otro de recepción de resultados*/
        ...
        argToThread->buffer_ignMap =
                (double **)malloc(argToThread->nroCombinacionesPorWorker *sizeof(double*));
        argToThread->buffer_flMap =
                (double **)malloc(argToThread->nroCombinacionesPorWorker*sizeof(double*));
        for(i = 0; i < argToThread->nroCombinacionesPorWorker ; i++){
            argToThread->buffer_ignMap[i] =
                    (double *)malloc(argToThread->infoMaps->Rows * argToThread->infoMaps->Cols *sizeof(double));
        ...
        pthread_cond_destroy(&(argToThread->varsMC->condpBufferIgnMap));
        pthread_cond_destroy(&(argToThread->varsMC->condcBufferFlMap));
        pthread_cond_destroy(&(argToThread->varsMC->condpBufferFlMap));
...
VarsMC * inicializarVarsMC(){
    ...
    pthread_cond_init(&(varsMC->condpBufferIgnMap),0);
    pthread_cond_init(&(varsMC->condcBufferFlMap),0);
    pthread_cond_init(&(varsMC->condpBufferFlMap),0);
    return varsMC;
...
void * enviarTrabajos(void * argToThread){
        ...
        int TAM_PAQUETE = args->tamPaquete;
        ...
        int tam_in;//tamaño del buffer de entrada
        ...
        if((args->nroCombinacionesWorkerRestante % TAM_PAQUETE) == 0){
                    nroDeEnviosWR = (int)(args->nroCombinacionesWorkerRestante / TAM_PAQUETE);
                    nroCombinaciones[args->nroDeWorkers-1] = (int *)malloc(nroDeEnviosWR*sizeof(int));
                    for(j = 0 ; j < nroDeEnviosWR ; j++){
                        nroCombinaciones[args->nroDeWorkers-1][j] = TAM_PAQUETE;
                    }//fin for
        }
        else{
            nroDeEnviosWR = ((int)(args->nroCombinacionesWorkerRestante / TAM_PAQUETE)) + 1;
                    nroCombinaciones[args->nroDeWorkers-1] = (int *)malloc(nroDeEnviosWR*sizeof(int));
                    for(j = 0 ; j < nroDeEnviosWR-1 ; j++){
                        nroCombinaciones[args->nroDeWorkers-1][j] = TAM_PAQUETE;
                    }//fin for
                    nroCombinaciones[args->nroDeWorkers-1][j] = (args->nroCombinacionesWorkerRestante % TAM_PAQUETE);
        }

        /*se asigna el nro de combinaciones a tratar al resto de los workers*/
        if((args->nroCombinacionesPorWorker % TAM_PAQUETE) == 0){
            nroDeEnvios = (int)args->nroCombinacionesPorWorker / TAM_PAQUETE;
                for(i = 0;i<(args->nroDeWorkers-1);i++){
                    nroCombinaciones[i] = (int *)malloc(nroDeEnvios*sizeof(int));
                    for(j = 0 ; j < nroDeEnvios ; j++){
                        nroCombinaciones[i][j] = TAM_PAQUETE;
                    }//fin for
                }//fin for
        }
        else{
            nroDeEnvios = ((int)(args->nroCombinacionesPorWorker / TAM_PAQUETE)) + 1;
            for(i = 0;i<(args->nroDeWorkers-1);i++){
                    nroCombinaciones[i] = (int *)malloc(nroDeEnvios*sizeof(int));
                    for(j = 0 ; j < (nroDeEnvios-1) ; j++){
                        nroCombinaciones[i][j] = TAM_PAQUETE;
                    }//fin for
                    nroCombinaciones[i][j] = (args->nroCombinacionesPorWorker % TAM_PAQUETE);
            }//fin for
        }

        for(i = 1 ; i < args->nroDeWorkers; i++){
            MPI_Send(&(args->nroCombinacionesPorWorker),1, MPI_INT,i,COMBPORWORKER_TAG,MPI_COMM_WORLD);
        }//fin for
        MPI_Send(&(args->nroCombinacionesWorkerRestante),1, MPI_INT,i,COMBPORWORKER_TAG,MPI_COMM_WORLD);
        //se arma un paquete con información sobre las matrices a utilizar durante las simulaciones
        tam_out = 2*sizeof(int) + 2*sizeof(double);
        outBuf = (char*)malloc(tam_out);
        position = 0;
        MPI_Pack(&(args->infoMaps->Rows),1,MPI_INT,outBuf,tam_out,&position,MPI_COMM_WORLD);
        MPI_Pack(&(args->infoMaps->Cols),1,MPI_INT,outBuf,tam_out,&position,MPI_COMM_WORLD);
        MPI_Pack(&(args->infoMaps->CellWd),1,MPI_DOUBLE,outBuf,tam_out,&position,MPI_COMM_WORLD);
        MPI_Pack(&(args->infoMaps->CellHt),1,MPI_DOUBLE,outBuf,tam_out,&position,MPI_COMM_WORLD);
        for(i = 1 ; i < args->nproc;i++){
            MPI_Send(outBuf,position, MPI_PACKED,i,MATRIZ_TAG,MPI_COMM_WORLD);
        }//fin for

        /*se le avisa a cada worker la cantidad de combinaciones que le corresponde*/
        for(i = 1 ; i < args->nroDeWorkers; i++){
            MPI_Send(&nroDeEnvios,1, MPI_INT,i,NRO_ENVIOS_TAG,MPI_COMM_WORLD);
            MPI_Send(nroCombinaciones[i-1],nroDeEnvios, MPI_INT,i,COMBINACIONES_TAG,MPI_COMM_WORLD);
        }//fin for
        MPI_Send(&nroDeEnviosWR,1, MPI_INT,i,NRO_ENVIOS_TAG,MPI_COMM_WORLD);
        MPI_Send(nroCombinaciones[i-1],nroDeEnviosWR, MPI_INT,i,COMBINACIONES_TAG,MPI_COMM_WORLD);
        //se calcula el tamaño del buffer de entrada
        tam_in = NRO_PARAMETROS;
        ...
        tam_out =nroCombinaciones[i][j]*tam_in*sizeof(double);//se asigna espacio para almacenar el siguiente paquete a enviar
        outBuf = (char*)malloc(tam_out);
...
void * recibirResultados(void * argToThread){
    ...
    tam_buf = 2*args->infoMaps->Rows*args->infoMaps->Cols*sizeof(double);
    tamMap = args->infoMaps->Rows*args->infoMaps->Cols;
    inBuf = (char *)malloc(tam_buf);
    /*se realiza un bucle hasta que se hayan obtenidos los resultados de todas las combinaciones*/
    while(comb < args->nroCombinaciones){
        ...
        for(np=1;np < args->nproc;np++){
            ...
            if(flag){
                ...
                while(args->varsMC->condicion_buffer_ignMap == args->nroCombinacionesPorWorker){//mientras el buffer esté lleno
                    pthread_cond_wait(&(args->varsMC->condpBufferIgnMap),&(args->varsMC->mutexBufferIgnMap));
                }//fin while
                ...
                while(args->varsMC->condicion_buffer_flMap == args->nroCombinacionesPorWorker){//mientras el buffer esté lleno
                    pthread_cond_wait(&(args->varsMC->condpBufferFlMap),&(args->varsMC->mutexBufferFlMap));
                }//fin while
                ...
                indexBuffer = (indexBuffer +1)%(args->nroCombinacionesPorWorker);
                comb++;
            }//fin if
        }//fin for
    }//fin while
...
void * sumarResultados(void * argToThread){
    ...
    double comb = 0;
    int i;
    cantCeldas = args->infoMaps->Rows * args->infoMaps->Cols;
    ...
    while(comb < args->nroCombinaciones){
        ...
        pthread_cond_signal(&(args->varsMC->condpBufferIgnMap));
        ...
        pthread_cond_signal(&(args->varsMC->condpBufferFlMap));
